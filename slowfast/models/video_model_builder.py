...
class FuseFastToSlow(nn.Module):
    ...
    def forward(self, x):
        x_s = x[0]
        # logger.info("Size x_s: {}".format((x_s.size())))
        x_f = x[1]
        # logger.info("Size x_f input: {}".format((x_f.size())))
        fuse = self.conv_f2s(x_f)
        # logger.info("Size x_f conv: {}".format((x_f.size())))
        fuse = self.bn(fuse)
        # logger.info("Size x_f bn: {}".format((x_f.size())))
        fuse = self.relu(fuse)
        # logger.info("Size x_f relu: {}".format((x_f.size())))
...
class SlowFast(nn.Module):
    ...
    def _construct_network(self, cfg):
        ...
        self.s1 = stem_helper.VideoModelStem(
            dim_in=cfg.DATA.INPUT_CHANNEL_NUM, ###[3, 3]
            dim_out=[width_per_group, width_per_group // cfg.SLOWFAST.BETA_INV],### [64, 8]
        ...
        self.s1_fuse = FuseFastToSlow(
            width_per_group // cfg.SLOWFAST.BETA_INV, ### 8
    ...
    def forward(self, x, bboxes=None):
        ...
        x = self.s4_fuse(x)
        x = self.s5(x)
        # logger.info("Size x(s5): {}".format((x[0].size())))        
        # logger.info("Size x(s5): {}".format((x[1].size())))
        if self.enable_detection:
            x = self.head(x, bboxes)
        else:
            # logger.info("Size x: {}".format((x[0].size())))            
            # logger.info("Size x: {}".format((x[1].size())))
            x = self.head(x)
        return x
